...
if ($_REQUEST['act'] == 'list')
{
    ...
    $sql = "SELECT article_id,title FROM " . $ecs->table('article');
    $articles = $db->getAll($sql);

    $smarty->assign('articles', $articles);
    $smarty->display('1_art_list.htm');
}
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article')." (title,content) values ('$title', '$content')";
    $rs = $db->query($sql);

    if ($rs) {
        echo "<script>alert('插入成功');location.href='1_art.php?act=list'</script>";
    } else {
        echo "<script>alert('插入失败');location.href='1_art.php?act=list'</script>";
    }
    die;
    /* 权限判断 */
